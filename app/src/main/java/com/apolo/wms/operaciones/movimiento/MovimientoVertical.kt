...
import com.apolo.wms.operaciones.movimiento.adapter.AdapterMovimientoVertical
...
import kotlinx.android.synthetic.main.list_movimiento_vertical.btCancelarBusqueda
import kotlinx.android.synthetic.main.list_movimiento_vertical.btnCancelarList
import kotlinx.android.synthetic.main.list_movimiento_vertical.etBuscarArticulo
import kotlinx.android.synthetic.main.list_movimiento_vertical.etCodigoBarraList
import kotlinx.android.synthetic.main.list_movimiento_vertical.rvOperaciones
...
class MovimientoVertical : AppCompatActivity() {
    ...
    companion object {
        ...
        suspend fun obtieneListaOperaciones() {
            ...
            var result = withContext(Dispatchers.IO){
                HttpRequest.call("", "movimiento/buscar_movimiento_vertical", formBody)
            }
            ...
            var respuestaJson = try {
                ...
                MainActivity.funciones.mensajeError(
                    context,
                    "Atencion",
                    "Error en la respuesta del servidor. (buscar_movimiento_vertical) Error ${e.message.toString()} !"
                )
                return
            }
...
//                rvOperacion.setHasFixedSize(true)
                CoroutineScope(Dispatchers.Main).launch {
                    rvOperacion.apply {
                        ...
                        adapter = AdapterMovimientoVertical(
                            context,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.list_movimiento_vertical)
...
// ultimo cambio realizado, descomentar del backup si cuando se de el visto bueno
//        etCodigoBarraList.setOnKeyListener(View.OnKeyListener { v, keyCode, event ->
//            if (keyCode == KeyEvent.KEYCODE_ENTER && event.action === KeyEvent.ACTION_UP) {
//                // Llamar a la función para abrir el modal
//                abreCuadroArticulo()
//
//                // Retener el foco en el RecyclerView
//                rvOperacion.findViewHolderForAdapterPosition(posicionOperacion)?.itemView?.requestFocus()
////                etCodigoBarraList.requestFocus()
//                return@OnKeyListener true // Prevenir que el foco se mueva automáticamente
//            }
//            false
//        })
        ...
        if (respuestaJson.has("rows")) {
            ...
            if (tr.codArticulo != codigo) {
                ...
                operacionesPendientes.forEachIndexed { index, it ->
                    ...
                    if (it.dirOrigen == etCodigoBarraList.text.toString() && it.codArticulo == codigo) {

                        var resu = false
                        if (it.relacion.trim() == "0") {
                            posicionOperacion = index
                            tr = operacionesPendientes[posicionOperacion]
                            resu=true
                        }
    ...
    fun confirmaMovimientoHorizontal() : Boolean {
        ...
        try {
            ...
            MainActivity.funciones.mensajeError(context, "Atencion", "Error en la respuesta del servidor. (confirma_movimiento_vertical) Error ${e.message.toString()} !")
            return false
        }
        ...
        if (respuestaJson.has("respuesta")) {
            ...
            if(respuesta == "X"){
                MainActivity.funciones.mensajeError(context, "Atencion", "Error al finalizar Movimiento Vertical")
            }else{
    ...
    fun validaDireccionMovimientoHorizontal(direccionDes: String) : Boolean {
        ...
        if (respuestaJson.has("respuesta")) {
            ...
            MainActivity.funciones.mensajeError(context, "Atencion", "Ocurrio un error en la respuesta del servidor. (validaDireccionMovVertical)")
            return false
        }
    ...
    fun obtieneListaOperacionesArticulo() {
        ...
        var result = HttpRequest.call("", "movimiento/buscar_movimiento_vertical_articulo", formBody)
        ...
        if (respuestaJson.has("rows")) {
            ...
            rvOperacion.layoutManager = LinearLayoutManager(
                context
            )
            ...
            val adapter = AdapterMovimientoVertical(
                context,
                operacionesPendientes,
                R.layout.card_view_movimiento_horizontal  )
